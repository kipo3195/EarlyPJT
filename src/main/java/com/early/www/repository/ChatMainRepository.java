...
public interface ChatMainRepository extends JpaRepository<ChatMain, Long>{
	...
	@Query(nativeQuery = true, value = "select chat_seq, chat_contents, chat_del_flag, chat_encrypt_key, chat_line_key, chat_receiver,"
			...
			+ " from ("
			+ " select a.*, e.name from tbl_chat_main as a join earlyuser e on a.chat_sender = e.username  where chat_room_key = :chatRoomKey and chat_type !='O' order by chat_seq desc limit 0, 20) as aa order by chat_seq asc")
